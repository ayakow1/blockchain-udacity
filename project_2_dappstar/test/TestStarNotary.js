...
it("can add the star name and star symbol properly", async () => {
  ...
  let tokenId = 11;
  let instance = await StarNotary.deployed();
  await instance.createStar("1 star", tokenId, {
    from: accounts[0],
  });
  //2. Call the name and symbol properties in your Smart Contract and compare with the name and symbol provided
  assert.equal(await instance.name.call(), "New Star");
  assert.equal(await instance.symbol.call(), "starUSD");
});
...
it("lets 2 users exchange stars", async () => {
  ...
  let tokenId2 = 13;
  let instance = await StarNotary.deployed();
  await instance.createStar("2 star", tokenId1, { from: accounts[0] });
  await instance.createStar("3 star", tokenId2, { from: accounts[1] });
...
it("lets a user transfer a star", async () => {
  ...
  let tokenId = 14;
  let instance = await StarNotary.deployed();
  await instance.createStar("4 star", tokenId, { from: accounts[0] });
...
it("lookUptokenIdToStarInfo test", async () => {
  ...
  let tokenId = 15;
  let instance = await StarNotary.deployed();
  await instance.createStar("5 star", tokenId, { from: accounts[0] });
